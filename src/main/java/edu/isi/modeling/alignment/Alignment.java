...
import edu.isi.modeling.learner.LinkCoherence;
...
import edu.isi.modeling.ontology.OntologyUpdateListener;
...
import edu.isi.modeling.rep.ColumnSubClassLink;
import edu.isi.modeling.rep.DataPropertyLink;
import edu.isi.modeling.rep.DataPropertyOfColumnLink;
...
public class Alignment implements OntologyUpdateListener {
	...
	public Alignment(OntologyManager ontologyManager) {
		...
		if (ModelingConfiguration.getKnownModelsAlignment()) {
			this.graphBuilder = 
					AlignmentGraph.getInstance(ontologyManager).getGraphBuilderClone();
		} else {
...
//	}
	...
	public List<LabeledLink> suggestLinks(String nodeId) {
		...
		incomingLinks = AlignmentGraph.getInstance(ontologyManager).
				getGraphBuilder().getIncomingLinks(nodeId);
		outgoingLinks = AlignmentGraph.getInstance(ontologyManager).
